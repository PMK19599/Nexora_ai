...
export default function Header() {
  ...
  return (
    <header className="flex h-16 items-center justify-between border-b bg-card px-4 md:px-6">
      ...
      <div className="flex items-center gap-2 md:hidden"><span className="text-xl">🧠</span><span className="font-bold text-primary">NeuroLearn</span></div>
      ...
      {showMobile && <div className="fixed inset-0 z-50 md:hidden"><div className="absolute inset-0 bg-black/50" onClick={() => setShowMobile(false)} /><nav className="absolute left-0 top-0 h-full w-64 bg-card p-4 shadow-lg"><div className="mb-6 flex items-center gap-2"><span className="text-2xl">🧠</span><span className="text-lg font-bold text-primary">NeuroLearn AI</span></div><ul className="space-y-1">{[{p:'/dashboard',l:'Dashboard',i:'📊'},{p:'/review',l:'Review',i:'🧠'},{p:'/career',l:'Career',i:'🚀'},{p:'/tutors',l:'Tutors',i:'👥'},{p:'/groups',l:'Groups',i:'📚'},{p:'/accessibility',l:'Access.',i:'♿'}].map(x=><li key={x.p}><NavLink to={x.p} onClick={()=>setShowMobile(false)} className={({isActive})=>`flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium ${isActive?'bg-primary text-primary-foreground':'hover:bg-accent'}`}><span>{x.i}</span><span>{x.l}</span></NavLink></li>)}</ul></nav></div>}
    </header>
  );
}
