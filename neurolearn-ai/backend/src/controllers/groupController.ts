...
export const scheduleMeeting = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => { try { res.status(201).json({ success: true, data: await svc.scheduleMeeting(req.params.groupId, req.user!._id.toString(), req.body) }); } catch (e) { next(e); } };
